    ...
    $(model).on('change', function() {
        var prodBucket = "";
        // console.log(window.location.protocol + '//' + window.location.hostname + '/' + (window.location.pathname.split("/"))[1] + '/'+ (window.location.pathname.split("/"))[2] + '/machine');
        $.ajax({
            type: 'POST',
            url: 'http://127.0.0.1:8000/v1/dealer/machine/prod/',
            headers: {"Access-Control-Allow-Origin": "*"},
            data: { pid: model.value,
                _token: "{{ csrf_token() }}",
            },
